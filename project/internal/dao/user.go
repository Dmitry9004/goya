...
func (dao *UserDAO) Save(user *models.User) (int, error) {
	...
	result, err := dao.db.ExecContext(dao.ctx, query, user.Username, password);
	if err != nil {
		log.Println(err)
		return 0, err
	}
...
func (dao *UserDAO) EqualPasswordsByUserId(id int, password string) bool {
...
func getPasswordHash(password string) string {
	passwordStr, _ := bcrypt.GenerateFromPassword([]byte(password), 14) 
