...
func main() {
	...
	if err = dao.CreateExpressionsTable(ctx, db); err != nil {
		log.Println(err)
	}
	...
	go services.ToSimpleTask(chanPostfixExpression, operationsTime, db, ctx, taskDAO)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Post("/calculate", func(w http.ResponseWriter, r *http.Request) {
			...
			id, err = dao.SaveExpression(ctx, db, expression, mu)
		...
		r.Get("/expressions", func(w http.ResponseWriter, r *http.Request) {
			...
			expressions, _ := dao.GetAllExpressionByUserId(ctx, db, int(userId))
		...
		r.Get("/expressions/{id}", func(w http.ResponseWriter, r *http.Request) {
			...
			resultExpression, error := dao.GetExpressionByIdAndUserId(ctx, db, idExpression, int(userId))
			if error != nil {
